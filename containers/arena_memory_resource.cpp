...
ArenaMemoryResourcePmr::ArenaMemoryResourcePmr(std::size_t initial_capacity)
    : arena{initial_capacity} {}
auto ArenaMemoryResourcePmr::do_allocate(std::size_t n_bytes, std::size_t alignment) -> void* {
    return arena.allocate(n_bytes, alignment);
}
void ArenaMemoryResourcePmr::do_deallocate(void* p, std::size_t n_bytes, std::size_t alignment) {
    return arena.deallocate(p, n_bytes, alignment);
}
auto ArenaMemoryResourcePmr::do_is_equal(std::pmr::memory_resource const& other) const noexcept -> bool {
    return this == &other;
}
}
