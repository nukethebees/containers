...
template <typename SortingFn>
class UnsignedIntSortingTest : public testing::Test {
  public:
    template <typename... Args>
    static auto call(Args&&... args) {
        return SortingFn().call(std::forward<Args>(args)...);
    }
};
TYPED_TEST_SUITE_P(UnsignedIntSortingTest);

TYPED_TEST_P(UnsignedIntSortingTest, Array5) {
    std::array<unsigned int, 5> in{10, 80, 30, 90, 40};
    std::array<unsigned int, 5> out{10, 30, 40, 80, 90};

    this->call(in.begin(), in.end());
    EXPECT_EQ(in, out);
}
TYPED_TEST_P(UnsignedIntSortingTest, Array20) {
    std::array<unsigned int, 20> in{22, 10, 11, 27, 67, 44, 76, 22, 74, 22, 84, 16, 20, 98, 38, 41, 8, 41, 75, 18};
    std::array<unsigned int, 20> out{8, 10, 11, 16, 18, 20, 22, 22, 22, 27, 38, 41, 41, 44, 67, 74, 75, 76, 84, 98};

    this->call(in.begin(), in.end());
    EXPECT_EQ(in, out);
}
TYPED_TEST_P(UnsignedIntSortingTest, Array200) {
    std::array<unsigned int, 200> in{
        ...
        4814,     85858};
    std::array<unsigned int, 200> out{
...
REGISTER_TYPED_TEST_SUITE_P(UnsignedIntSortingTest, Array5, Array20, Array200);

template <typename SortingFn>
class SignedIntSortingTest : public testing::Test {
  public:
    template <typename... Args>
    static auto call(Args&&... args) {
        return SortingFn().call(std::forward<Args>(args)...);
...
TYPED_TEST_SUITE_P(SignedIntSortingTest);

TYPED_TEST_P(SignedIntSortingTest, Array48) {
    std::array<int, 48> in{-7, 12, -1, 0, 5, -9, 8, -3, 21, -5, 11, -2, 15, 1,  0,  -6, 7,  -4, 19, -8, -1, 6,  3,  -10,
                           20, 9,  -7, 2, 0, -1, 4, 1,  13, -3, 16, 14, -2, 18, 22, -4, 10, -5, 17, -6, -9, -8, 11, 3};
    std::array<int, 48> out{-10, -9, -9, -8, -8, -7, -7, -6, -6, -5, -5, -4, -4, -3, -3, -2,
...
TYPED_TEST_P(SignedIntSortingTest, Array10) {
    std::array<int, 10> in{1, 2, 1, 1, 0, 0, 0, -1, -1, -1};
    std::array<int, 10> out{-1, -1, -1, 0, 0, 0, 1, 1, 1, 2};

    this->call(in.begin(), in.end());
    EXPECT_EQ(in, out);
}
TYPED_TEST_P(SignedIntSortingTest, Array20) {
    std::array<int, 20> in{22, 10, 11, 27, 67, 44, 76, 22, 74, 22, 84, 16, 20, 98, 38, 41, 8, 41, 75, 18};
    std::array<int, 20> out{8, 10, 11, 16, 18, 20, 22, 22, 22, 27, 38, 41, 41, 44, 67, 74, 75, 76, 84, 98};

    this->call(in.begin(), in.end());
    EXPECT_EQ(in, out);
}
TYPED_TEST_P(SignedIntSortingTest, Array8) {
    std::array<int, 8> in{42, 7, 19, 84, 3, 56, 29, 11};
    std::array<int, 8> out{3, 7, 11, 19, 29, 42, 56, 84};

    this->call(in.begin(), in.end());
    EXPECT_EQ(in, out);
}
TYPED_TEST_P(SignedIntSortingTest, Array5) {
    std::array<int, 5> in{10, 80, 30, 90, 40};
    std::array<int, 5> out{10, 30, 40, 80, 90};

    this->call(in.begin(), in.end());
    EXPECT_EQ(in, out);
}

REGISTER_TYPED_TEST_SUITE_P(SignedIntSortingTest, Array48, Array10, Array20, Array8, Array5);
