...
TEST(array, const_iterators) {
    ml::array<int, 3> const values{1, 2, 3};
...
TEST(array, const_methods_return_const_references) {
    ...
    if constexpr (!std::is_same_v<FrontType, ExpectedFrontType>) {
        GTEST_FAIL() << std::format(
            "front() does not return {}, got: {}", typeid(ExpectedFrontType).name(), typeid(FrontType).name());
    }
    if constexpr (!std::is_same_v<BackType, ExpectedBackType>) {
        GTEST_FAIL() << std::format(
            "back() does not return {}, got: {}", typeid(ExpectedBackType).name(), typeid(BackType).name());
    }
    if constexpr (!std::is_same_v<SubscriptType, ExpectedSubscriptType>) {
        GTEST_FAIL() << std::format("operator[] does not return {}, got: {}",
                                    typeid(ExpectedSubscriptType).name(),
                                    typeid(SubscriptType).name());
    }
    if constexpr (!std::is_same_v<DataType, ExpectedDataType>) {
        GTEST_FAIL() << std::format(
            "data() does not return {}, got: {}", typeid(ExpectedDataType).name(), typeid(DataType).name());
    }
